import argparse,time,cv2,torch,csv,sqlite3,smtplib,mimetypes,folium
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for c in det[:, -1].unique():
                    ...
                    if n>0:
                        items.append(loc)
                        crsr.execute("INSERT INTO Location_Table (sl_no, lat, lon) VALUES (?, ?, ?)",(len(items), loc.lat,loc.lon))
...
#Display on map
    ...
    my_mapp.save("maps\stranded_people.html ")
    ...
    try:
        with open(filename, 'w') as f:
            writer = csv.writer(f)
            for item in items:
                writer.writerow([item.alt, item.lat, item.lon])
...
if __name__ == '__main__':
    ...
    print("Connecting to vehicle on: %s" % ('com7',))
    ...
    sql_command = """DROP TABLE Location_Table;"""
    crsr.execute(sql_command)

    #Create a new table
    sql_command = """CREATE TABLE Location_Table ( 
    ...
    msg["From"] = emailfrom
    msg["To"] = emailto
    msg["Subject"] = "Stranded people! ples help"
    msg.preamble = "Stranded people! ples help"
